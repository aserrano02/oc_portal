...
export class CategoryPage extends PageTransitionsMixin(LitElement) {
  ...
  static inbounds = {
    ...
    _dislikedRecipes: { channel: 'disliked-recipes' },
  };
    ...
    return html`
      <div class="page-header">
        <ul class="page-header-sup">
          ...
          <li>
            <md-outlined-button
              aria-label="hate recipes"
              href="#!/hate-recipes"
              @click="${() => this._navigateToHateRecipes}"
            >
              <md-icon filled slot="icon">heart_broken</md-icon>
              ${this._dislikedRecipes.size}
              <span class="md-outlined-button-text">heat recipes</span>
            </md-outlined-button>
          </li>
        </ul>
  ...
  _navigateToHateRecipes(ev: CustomEvent) {
    ev.preventDefault();
    ev.stopPropagation();
    this.pageController.navigate('hate-recipes');
  }
  ...
  async _addDisLikedRecipes(ev: CustomEvent, recipe: Recipe) {
    const hateRecipe = (await getMealDetailsById(recipe.idMeal)).meals[0];
    (ev.target as MdOutlinedIconButton).selected
      ? this._dislikedRecipes.add(hateRecipe)
      : this._delete(hateRecipe.idMeal, this._dislikedRecipes);
    this.pageController.publish('liked-recipes', this._dislikedRecipes);
    this.requestUpdate();
  }
