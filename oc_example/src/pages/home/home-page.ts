...
export class HomePage extends PageTransitionsMixin(PageMixin(LitElement)) {
  ...
  disconnectedCallback() {
    ...
    this.unsubscribe('disliked-recipes');
      ...
      <page-layout>
        <div class="home-header">
          ...
          <md-outlined-button
            ...
            @click="${() => this.navigate('hate-recipes')}"
          >
            <md-icon filled slot="icon">heart_broken</md-icon>
            ${this._dislikedRecipes?.size}
            <span class="md-outlined-button-text">hate recipes</span>
          </md-outlined-button>
        </div>
    ...
    return html`
      ...
      <div class="banner-text">
        ...
        <div class="banner-text-actions">
          ...
          <md-outlined-icon-button
            aria-label="Add recipe to hate"
            toggle
            @click="${(ev: CustomEvent) =>
              this._randomRecipe && this._addDisLikedRecipes(ev, this._randomRecipe)}"
            ?selected="${Boolean(
              this._dislikedRecipes
                ? [...this._dislikedRecipes].find(item => item.idMeal === this._randomRecipe?.idMeal)
                : false,
            )}"
          >
  ...
  _addDisLikedRecipes(ev: CustomEvent, recipe: Recipe) {
    ...
    (ev.target as MdOutlinedIconButton).selected
      ...
      : this._delete(recipe, this._dislikedRecipes);
    this.publish('disliked-recipes', this._dislikedRecipes);
    this.requestUpdate();
  }
